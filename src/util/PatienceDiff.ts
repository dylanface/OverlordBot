...
export interface PatienceDiffOutput {
  lines: Array<PatienceDiffLine>;
  lineCountDeleted: number;
  lineCountInserted: number;
  lineCountMoved: number;
  aMove?: Array<string>;
  aMoveIndex?: Array<number>;
  bMove?: Array<string>;
  bMoveIndex?: Array<number>;
}
...
export function patienceDiff(
  ...
  //
  function findUnique(arr: Array<any>, lo: number, hi: number) {
    var lineMap = new Map();
  ...
  // inserted.
  let result: Array<PatienceDiffLine> = [];
  let deleted = 0;
  let inserted = 0;
  ...
  let aMove: Array<string> = [];
  let aMoveIndex: Array<number> = [];
  let bMove: Array<string> = [];
  let bMoveIndex: Array<number> = [];
 ...
 *
 */
export function patienceDiffPlus(aLines: Array<string>, bLines: Array<string>) {
  ...
  do {
    ...
    if (!aMove || !bMove) {
      throw new Error("patienceDiffPlus: missing aMove or bMove");
    }
    ...
    subDiff.lines.forEach((v, i) => {
      ...
      if (!aMoveIndex || !bMoveIndex) {
        throw new Error("patienceDiffPlus: missing aMoveIndex or bMoveIndex");
      }
      if (!aMoveNext || !bMoveNext) {
        throw new Error("patienceDiffPlus: missing aMoveNext or bMoveNext");
      }
      if (!aMoveIndexNext || !bMoveIndexNext) {
        throw new Error(
          "patienceDiffPlus: missing aMoveIndexNext or bMoveIndexNext"
        );
      }
