...
const { OverlordEvent } = require("./EventLogger");
...
class ModerationLogger {
  ...
  async publish(guild, event) {
    ...
    if (type === "mass-ban") {
      ...
      var embed = this.#massModerationEventToEmbed(
        moderator,
        type,
        suspects,
        reason
      ...
      var embed = this.#singularModerationEventToEmbed(
        moderator,
        type,
        suspect,
        reason
  ...
  #singularModerationEventToEmbed = (moderator, action, suspect, reason) => {
    const registryEmbed = new EmbedBuilder()
      ...
      .setAuthor({
        name: `${suspect.tag}`,
        iconURL: suspect.displayAvatarURL({ dynamic: true }),
      })
      .addFields({
        name: "Action:",
