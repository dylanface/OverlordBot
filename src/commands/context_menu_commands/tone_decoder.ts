import { OverlordContextMenuCommand } from "../../types/Overlord";
...
export = <OverlordContextMenuCommand>{
  enabled: false,
  name: "tone_decoder",
  ...
  execute: async (client, interaction) => {
    if (!interaction.targetMessage || !interaction.targetMessage.content)
      return;
    ...
    // if (interaction.targetMessage.type !== "DEFAULT")
    //   return interaction.reply(
    //     "You cannot use tone_decoder on interaction replies."
    //   );
    if (interaction.targetMessage.author.bot)
      return interaction.reply("You cannot use tone_decoder on bot messages.");
    if (messageContent.length < 3)
      return interaction.reply(
        "You cannot use tone_decoder on messages shorter than 3 characters."
      );
    if (messageContent.length > 1700)
      return interaction.reply(
        "You cannot use tone_decoder on messages longer than 1700 characters."
      );
    if (messageContent.includes("```"))
      return interaction.reply(
        "You cannot use tone_decoder on messages with code blocks."
      );
