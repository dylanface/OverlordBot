...
export = <OverlordEvent>{
  ...
  async execute(client, oldMessage, newMessage) {
    ...
    if (!messageLogsChannel.isTextBased())
      throw new Error("Channel is not text based");
    await messageLogsChannel
      .send({ embeds: [registryEmbed], components: [mobileLink as any] })
      .catch((error) => {
...
/**
 * Run a PatienceDiff algorithm to compare two plain text messages extracted from the discord message objects.
 */
function compareMessages(newMessage: string, oldMessage: string) {
 ...
 */
function highlightInsertedLines(diff: PatienceDiffOutput) {
 ...
 */
function highlightDeletedLines(diff: PatienceDiffOutput) {
  let closeIndexGrouping = [];
  let lastWord = {};
